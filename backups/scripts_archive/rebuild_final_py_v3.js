...
<head>
    ...
    <style>
        ...
        body { background-color: #f8f9fa; font-family: "Microsoft JhengHei", sans-serif; overflow-x: hidden !important; }
        .main-wrapper { display: flex; min-height: 100vh; }
        .sidebar { width: 280px; background: #fff; border-right: 1px solid #dee2e6; position: fixed; top: 0; bottom: 0; left: 0; z-index: 1000; display: flex; flex-direction: column; transition: transform 0.3s; height: 100vh; }
        .sidebar-header { background: #212529; color: #fff; padding: 15px; border-bottom: 1px solid #dee2e6; flex-shrink: 0; }
        .sidebar-header h5 { font-size: 1.25rem; font-weight: bold; color: #fff; margin-bottom: 5px; }
        #progress-stats { font-size: 1.1rem; font-weight: bold; color: #ffc107; }
        .sidebar-content { flex: 1; overflow-y: auto !important; padding: 15px; }
        .content-area { flex: 1; margin-left: 280px; padding: 0; transition: margin-left 0.3s; overflow-x: hidden !important; }
        ...
        .q-node { width: 42px; height: 42px; display: flex; align-items: center; justify-content: center; border: 1px solid #dee2e6; border-radius: 6px; background-color: #fff; cursor: pointer; font-size: 0.85rem; margin: 3px; flex-shrink: 0; }
        ...
        .q-node.active { border: 2.5px solid #0d6efd; transform: scale(1.05); font-weight: bold; z-index: 5; }
        .progress-grid { display: flex; flex-wrap: wrap; gap: 4px; justify-content: flex-start; }
        ...
        .zoom-btn { width: 52px; height: 52px; border-radius: 50%; background: #212529; color: white; border: 2px solid #fff; font-size: 26px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 15px rgba(0,0,0,0.5); cursor: pointer; user-select: none; }
        #question-container { transform-origin: top center; transition: none !important; }
        
        .matching-wrapper { position: relative; margin: 30px 0; width: 100%; user-select: none; touch-action: pan-y pinch-zoom; overflow: visible !important; }
        ...
        .match-dot { width: 22px; height: 22px; border: 1.5px solid #333; border-radius: 50%; margin: 0 15px; display: flex; align-items: center; justify-content: center; background: #fff; flex-shrink: 0; position: relative; }
        ...
        .side-nav-btn { position: fixed; top: 55%; width: 25px; height: 65px; background: rgba(0,0,0,0.5); color: white; display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 2000; transform: translateY(-50%); border: none; }
        ...
        @media (max-width: 992px) { .sidebar { transform: translateX(-100%); } .sidebar.active { transform: translateX(0); } .content-area { margin-left: 0; } .side-nav-prev { left: 0; } .matching-columns { gap: 30px; } }
    ...
    <div class="main-wrapper">
        <nav class="sidebar" id="sidebar">
            <div class="sidebar-header">
                <h5>REPLACE_TITLE 認證練習</h5>
                <div id="progress-stats">✅0 ❌0 🟠0 / REPLACE_TOTAL</div>
            </div>
            <div class="sidebar-content"><div class="progress-grid" id="progress-grid"></div></div>
        </nav>
        ...
        <main class="content-area"><div class="container-fluid" style="width: calc(100% - 60px); margin: 20px auto;"><div id="question-container"></div></div></main>
    </div>
...
<script>
    ...
    function renderMatchingQuestion(index) {
        ...
        item.left.forEach((text, lIdx) => {
            ...
            html += `<div class="match-item"><div>\${text}</div><div class="match-dot" id="dot-left-\${lIdx}" onmousedown="\${completed?'':`handleDragStart(event,'left',\${lIdx})`}"><div style="background:\${dotC}"></div></div></div>`;
        });
    ...
    for subj in config['subjects']:
        try:
            ...
            with open(prac_path, 'w', encoding='utf-8') as f:
                f.write(prac_top.replace('REPLACE_TITLE', subj['title']).replace('REPLACE_TOTAL', String(len(quiz_obj))).replace('REPLACE_SUBJECT_ID', subj['id']))
                f.write(json.dumps(quiz_obj, ensure_ascii=False))
...
fs.writeFileSync('final_clean_repair.py', fullPyCode, 'utf-8');
console.log('SUCCESS: final_clean_repair.py fully restored with original UI and new features.');
