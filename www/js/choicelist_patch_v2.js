...
(function() {
    ...
    const stripCodeTags = (str) => {
        if (!str) return "";
        let text = Array.isArray(str) ? str.join('\n') : String(str);
        return text.replace(/<pre><code.*?>/g, "").replace(/<\/code><\/pre>/g, "").replace(/<code.*?>/g, "").replace(/<\/code>/g, "");
    };

    const highlightHardened = (text, lang = "python") => {
    ...
    window.renderChoiceListQuestion = function(index) {
        ...
        if (slotData) {
            ...
            slotData.forEach(line => {
                let processedLine = Array.isArray(line) ? line.join('\n') : String(line);
                if (Array.isArray(line) && line.length >= 2) {
                    const first = String(line[0]).trim();
                    const last = String(line[line.length - 1]).trim();
                    if (first.startsWith('<code class="language-') && last.endsWith('</code>')) {
                        processedLine = '<pre>' + processedLine + '</pre>';
                    }
                }
