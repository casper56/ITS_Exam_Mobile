# ITS Python 認證題庫統計分析 (2026-02-26)

## 1. 題庫規模與組成
*   **總題數**：190 題
*   **官方版本 (1-99)**：99 題 (包含今日新增之 8 題鑑定考點)
*   **補充版本 (100-190)**：91 題

## 2. 題型分佈
*   **單選題 (Single Choice)**：117 題
*   **複選題 (Multiple Choice)**：11 題
*   **多重下拉/填空 (Multi-option)**：62 題

## 3. 今日新增 (ID 92-99) 考點摘要
今日新增 8 題官方鑑定考點，主要針對 2025 年新版綱要進行補強：
*   **系統應用**：`sys.argv` 命令列參數索引與 `math.fabs` 數學運算。
*   **測試開發**：`unittest` 單元測試架構與 `assertEqual`/`assertIs` 斷言區分。
*   **文件化**：`Docstring` 標準位置與 `datetime` 進階格式化。
*   **邏輯陷阱**：`input()` 字串與 `dict` 整數鍵值的類型匹配錯誤。

## 4. 考點分佈 (依題數排序)
1.  **D1 資料型別與運算子**：46 題 —— **核心章節**
2.  **D4 函式與註解**：34 題
3.  **D2 流程控制與判斷**：31 題
4.  **D3 輸入輸出與檔案**：30 題
5.  **D6 模組與常用工具**：21 題
6.  **D5 錯誤處理與測試**：20 題
7.  **D0 官方鑑定補強**：8 題 (今日新增)
