...
<head>
    ...
    <style>
        ...
        .question-body {{ padding: 20px 25px; font-size: 1rem; overflow: visible !important; background-color: #fff; color: #212529; }}
        .option-item {{ list-style: none; margin-bottom: 8px; padding: 10px 15px; border: 1px solid #e9ecef; border-radius: 8px; cursor: pointer; transition: all 0.2s; background-color: #fff; }}
        ...
        .sub-opt-container {{ padding: 8px 12px; border: 1px solid #dee2e6; border-radius: 6px; cursor: pointer; background: #fff; transition: all 0.2s; }}
        .sub-opt-container.selected {{ background-color: #cfe2ff; border-color: #0d6efd; color: #084298; font-weight: bold; }}
        .sub-question-label {{ font-weight: bold; margin-top: 15px; margin-bottom: 8px; color: #212529; border-left: 4px solid #198754; padding-left: 10px; }}
        ...
        code {{ font-family: Consolas, Monaco, monospace; color: #212529; background-color: #fff; padding: 2px 4px; border-radius: 4px; border: 1px solid #eee; white-space: pre-wrap !important; word-break: break-all !important; overflow: visible !important; }}
        pre {{ white-space: pre-wrap !important; word-break: break-all !important; overflow: visible !important; margin: 0; background-color: #fff; color: #212529; border: none; }}
...
<script>
    ...
    function startExam() {{
        ...
        // 1. Minimum 3 questions per category (at least 5% of 50 is 2.5, round up to 3)
        ...
        for (let q of remainingPool) {{
            ...
            const cat = q.category || 'D7_其他進階題型';
            if (cat === d1Key) {{
                const d1Count = selected.filter(sq => sq.category === d1Key).length;
                if (d1Count < 17) {{
                    selected.push(q);
                }}
            }} else {{
