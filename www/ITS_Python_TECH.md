...
本文件是根據 `www/ITS_Python/questions_ITS_python.json` 題庫 (總計 187 題) 的最新分析結果所編寫。旨在協助考生從「代碼邏輯」與「語法細節」兩個維度掌握 ITS Python 認證。
...
*   **比較運算 (Comparison)**：注意 `==` (值相等) 與 `is` (物件識別/記憶體位置相同) 的區別。
...
## D2_流程控制與判斷：邏輯漏斗
### 1. 判斷邏輯順序：漏斗式過濾 (Funneling)
*   **核心原則**：當多個 `elif` 條件有重疊時，必須將**「最嚴苛/最細分」**的條件放在最前面。
*   **深度分析**：Python 的 `if-elif` 結構在命中第一個 True 條件後就會跳出。
    *   **正確範例**：先檢查 `age < 13` (童星) 再檢查 `age < 18` (青少年)。
    *   **錯誤範例**：先檢查 `age < 18` 再檢查 `age < 13`。這會導致 10 歲的孩子在第一關就命中 `age < 18` 並執行對應區塊，永遠無法進到 `age < 13` 的邏輯中。
*   **None 檢查保護**：執行比較前，務必先判斷變數是否為 `None`，以避免 runtime 錯誤。
*   **縮排 (Indentation)**：Python 高度依賴縮排，必須一致（4 個空格或 1 個 Tab），混合使用或層級錯誤會導致 `IndentationError`。
...
| **'w+'** | 讀寫 | **自動建立檔案** 且可讀寫，不會引發 `IOError`。 |

### 2. 命令列參數與輸出
*   `sys.argv`：列表格式。`sys.argv[0]` 為腳本名稱。
...
## D6_模組與常用工具：核心套件
### 1. Math 模組 (數值計算)
*   `math.floor(x)`：下向取整（不大於 x 的最大整數）。
*   `math.ceil(x)`：向上取整。
*   `math.fmod(x, y)`：Python 版的餘數計算，處理浮點數更精確。
*   `math.frexp(x)`：傳回 (尾數, 指數)。

### 2. Datetime 模組 (日期處理)
*   **時間格式化 (ID 88)**：使用 `strftime` 或 `.format()` 搭配格式化代碼。
    *   `%B`：月份的**完整名稱** (例如: April)。
    *   `%b`：月份的簡寫 (例如: Apr)。
    *   `%d`：兩位數的日期 (01-31)。
    *   `%y`：**兩位數**年份 (例如: 17)。
    *   `%Y`：四位數年份 (例如: 2017)。
    *   **考點範例**：`{:%B-%d-%y}` 會輸出如 `APRIL-07-17` 的格式。
