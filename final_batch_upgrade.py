...
subjects = [
    ...
    {'dir': 'www/ITS_Python', 'json': 'questions_ITS_python.json', 'title': 'ITS Python Programming 模擬考試'}
]
...
for subj in subjects:
    try:
        ...
        with open(mock_path, 'wb') as f:
            f.write(html_top.replace('REPLACE_TITLE', subj['title']).encode('utf-8'))
            f.write(json_bytes)
            f.write(b";")
            f.write(html_bottom.encode('utf-8'))
        print(f"Full Upgrade Successful: {mock_path}")
    except Exception as e:
        print(f"Failed {subj['dir']}: {e}")

print("Final All-Subject V3.4 Upgrade Completed!")
