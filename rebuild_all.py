...
# 2. 準備 HTML 模板的前半部 (CSS 樣式)
html_path = 'www/ITS_Python/ITS_Python.html'
with open(html_path, 'r', encoding='utf-8') as f:
    full_content = f.read()

# 找到 const quizData = 的位置作為切點
cut_idx = full_content.find('const quizData = ')
if cut_idx == -1:
    print("ERROR: cut point not found")
    exit()

head_part = full_content[:cut_idx]
...
') : String(content);
        ...
        opts.forEach((optStr, optIdx) => {
            if (String(optStr).includes('|')) {
                ...
                optStr.split('|').forEach((s, subIdx) => { 
                    sHtml += `<div class="sub-opt-container p-2 border rounded bg-light" onclick="checkSubAnswer(this, ${index}, ${optIdx}, ${subIdx}, event)" style="cursor:pointer; font-size:0.9rem"><input class="form-check-input" type="radio" name="q${index}_opt${oIdx}" id="o${optIdx}_s${subIdx}"> ${s}</div>`; 
                });
    ...
    function updateUI() {
        const stats = document.getElementById('progress-stats'); if (stats) stats.innerHTML = `✅${correctSet.size} ❌${incorrectSet.size} / ${quizData.length}`;
...
print("SUCCESS: Rebuilt ITS_Python.html from source. Syntax is clean.")
