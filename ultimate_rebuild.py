...
# 1. 抓取目前的 quizData (這是目前唯一有價值的變動資料)
start_marker = 'const quizData = ['
end_marker = '];'
s_idx = content.find(start_marker)
e_idx = content.find(end_marker, s_idx)
quiz_data_json = content[s_idx + len(start_marker) - 1 : e_idx + 1]
...
# 3. 執行全量替換 (尋找最後一個 <script> 區塊)
start_tag = '<script>'
end_tag = '</script>'
script_start = content.rfind(start_tag)
script_end = content.rfind(end_tag)

if script_start != -1 and script_end != -1:
    final_content = content[:script_start] + new_js + content[script_end + len(end_tag):]
