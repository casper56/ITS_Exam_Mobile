...
# V3.4 完美模板 (包含所有功能與修復)
# 注意：這裡使用 {json_data} 和 {exam_title} 作為替換點
...
<div id="exam-ui">
    <header class="exam-header d-flex justify-content-between align-items-center">
        <div><h5 class="m-0">{exam_title}</h5><small id="q-progress">1 / 50</small></div>
...
for subj in subjects:
    try:
        ...
        if not os.path.exists(json_file):
            print(f"Skipping {subj['dir']} (JSON not found)")
            continue
        ...
        # 生成 HTML (分段寫入以避免記憶體錯誤或 Python 字串限制)
        # 這裡因為我們用了 format，所以直接一次性生成
        final_content = html_template.format(exam_title=subj['title'], json_data=j_str)
        ...
        print(f"Upgraded: {mock_path}")
        
    except Exception as e:
        print(f"Error processing {subj['dir']}: {e}")

print("Batch upgrade completed!")
