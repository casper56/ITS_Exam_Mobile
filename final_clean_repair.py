    ...
    <style>
        ...
        @media print { 
            ...
            * { overflow: visible !important; max-height: none !important; height: auto !important; }
            ...
            pre, code { white-space: pre-wrap !important; word-break: break-all !important; border: none !important; font-size: 1.0rem !important; }
...
<script>
    ...
    function prepareAndPrint(onlyMistakes = false) {
        ...
        targetItems.forEach(({ item, idx }) => {
            ...
            div.innerHTML = `<div class="review-q-text"><b>${idx+1}.</b> ${processContent(cleanQ, item)}</div>${imgHtml}<div class="review-opts" style="margin-left:0">${optHtml}</div><div class="review-ans">正確答案：${ansText}</div><div class="review-exp">${processContent(item.explanation || '暫無解析。', item)}</div>`;
