...
<head>
    ...
    <style>
        ...
        /* 表格樣式：黑色實線邊框 */
        .q-table { width: 100%; border-collapse: collapse; margin: 15px 0; table-layout: fixed; border: 1px solid #000 !important; font-size: 0.9rem; line-height: 1.2; }
        .q-table th, .q-table td { border: 1px solid #000 !important; padding: 10px 8px; vertical-align: top; word-break: break-all; color: #000; }
...
<head>
    ...
    <style>
        ...
        .question-body { padding: 20px; font-size: 1.05rem; word-wrap: break-word; word-break: normal; overflow-x: hidden; }
    ...
    function prepareAndPrint(onlyMistakes = false) {
        ...
        targetItems.forEach(({ item, idx }) => {
            ...
            if (Array.isArray(cleanQ)) {
                ...
                cleanQ = String(cleanQ).replace(/^\d+\.\s*/, '');
            }
