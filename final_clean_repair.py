...
<script>
    ...
    window.renderChoiceListQuestion = function(index) {
        ...
        const displayAnsHtml = correctAnsArr.map((val, i) => {
            const optIdx = (typeof val === 'number') ? (val - 1) : -1;
            const text = (optIdx >= 0 && item.options[optIdx]) ? item.options[optIdx] : val;
            return `<div class="mt-2 p-2 border-start border-success border-4 bg-light small" style="white-space:pre-wrap; font-family:monospace;"><b>第 ${i+1} 步：</b>${text}</div>`;
        }).join('');
