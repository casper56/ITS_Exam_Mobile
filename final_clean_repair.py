...
<div id="exam-ui">
    <header class="exam-header d-flex justify-content-between align-items-center">
        ...
        <div>
            <button class="btn btn-warning btn-sm me-2 no-print" onclick="resetCurrentQuestion()">重設問題</button>
...
<script>    function prepareMockPrint() {
    ...
    mock_bottom_tmpl = r"""
        ...
        // 左側：[文字] [圓圈]
        item.left.forEach((rawText, lIdx) => {
            let text = Array.isArray(rawText) ? rawText.join('\n') : String(rawText);
            if (Array.isArray(rawText) && rawText.length >= 2) {
                const first = String(rawText[0]).trim();
                const last = String(rawText[rawText.length - 1]).trim();
                if (first.startsWith('<code class="language-') && last.endsWith('</code>')) {
                    text = '<pre>' + text + '</pre>';
                }
            }
        ...
        item.right.forEach((text, grpIdx) => {
            const processRightItem = (rawT, idx) => {
                let t = Array.isArray(rawT) ? rawT.join('\n') : String(rawT);
                if (Array.isArray(rawT) && rawT.length >= 2) {
                    const first = String(rawT[0]).trim();
                    const last = String(rawT[rawT.length - 1]).trim();
                    if (first.startsWith('<code class="language-') && last.endsWith('</code>')) {
                        t = '<pre>' + t + '</pre>';
                    }
                }
    ...
    function initExam() {
        ...
        // 動態計算目標題數 (若無補充題則全出官方題)
        let TARGET_OFF_COUNT = 59;
        let TARGET_SUPP_COUNT = EXAM_LIMIT - TARGET_OFF_COUNT;
        
        if (suppPoolTotal.length === 0) {
            TARGET_OFF_COUNT = EXAM_LIMIT;
            TARGET_SUPP_COUNT = 0;
        } else if (suppPoolTotal.length < TARGET_SUPP_COUNT) {
            TARGET_SUPP_COUNT = suppPoolTotal.length;
            TARGET_OFF_COUNT = EXAM_LIMIT - TARGET_SUPP_COUNT;
        }
    ...
    function renderQuestion(index, scrollTop = true) {
        ...
        options.forEach((opt, optIdx) => {
            let processedOpt = Array.isArray(opt) ? opt.join('\n') : String(opt);
            if (Array.isArray(opt) && opt.length >= 2) {
                const first = String(opt[0]).trim();
                const last = String(opt[opt.length - 1]).trim();
                if (first.startsWith('<code class="language-') && last.endsWith('</code>')) {
                    processedOpt = '<pre>' + processedOpt + '</pre>';
                }
            }
    ...
    function selectOption(optIdx) {
        ...
        if (item.type === 'multiple') {
            ...
            if (userAnswers[currentIndex].includes(optIdx)) return;
            userAnswers[currentIndex].push(optIdx);
        } else {
            if (userAnswers[currentIndex] !== undefined && userAnswers[currentIndex] !== null) return;
    ...
    function selectSub(qIdx, subIdx) {
        ...
        if (userAnswers[currentIndex][qIdx] !== undefined) return;
