...
for subj in subjects:
    try:
        ...
        # 採取分段寫入，完全避開 Python 內部的字串格式化邏輯
        with open(mock_path, 'w', encoding='utf-8') as f:
            f.write(html_top_part.replace('REPLACE_TITLE', subj['title']))
            f.write(j_str)
            f.write(";")
            f.write(html_bottom_part)
            
        print(f"Fixed & Rebuilt: {mock_path}")
        
    except Exception as e:
        print(f"Error processing {subj['dir']}: {e}")

print("Safe batch upgrade completed!")
