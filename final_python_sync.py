...
q26 = {
    "id": 26,
    "type": "matching",
    "question": ["<pre><code class="language-python">【CH03-5】你有以下清單結構：alph = "abcdefghijklmnopqrstuvwxyz"
以下各個程式碼片段的結果各是如何？
...
q48 = {
    "id": 48,
    "type": "matching",
    "question": ["<pre><code class="language-python">【CH05-6】你設計一個 Python 程式來檢查使用者輸入的數字是 1 位數、 2 位數還是 2 位數以上。
...
def sync(path):
    ...
    if 'const quizData = [' in content:
        ...
        json_data = json.loads(content[s + len(start_tag) - 1 : e + 1])
        for i in range(len(json_data)):
            if json_data[i]['id'] == 26: json_data[i] = q26
            if json_data[i]['id'] == 48: json_data[i] = q48
        new_content = content[:s + len(start_tag) - 1] + json.dumps(json_data, ensure_ascii=False, indent=4) + content[e+1:]
        with open(path, 'w', encoding='utf-8') as f: f.write(new_content)
        print(f"Synced HTML: {path}")
    elif path.endswith('.json'):
        data = json.loads(content)
        for i in range(len(data)):
            if data[i]['id'] == 26: data[i] = q26
            if data[i]['id'] == 48: data[i] = q48
        with open(path, 'w', encoding='utf-8') as f: json.dump(data, f, ensure_ascii=False, indent=4)
        print(f"Synced JSON: {path}")

sync('www/ITS_Python/questions_ITS_python.json')
sync('www/ITS_Python/ITS_Python.html')
sync('www/ITS_Python/mock_v34.html')
