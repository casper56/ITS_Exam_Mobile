...
<script>
    ...
    function resetProgress() { 
        if(confirm('確定清除紀錄嗎？')) { 
            try { 
                localStorage.removeItem(CORR_KEY); localStorage.removeItem(INCORR_KEY); localStorage.removeItem(CORR_EDIT_KEY); localStorage.removeItem(INDEX_KEY); localStorage.removeItem(ANSWERS_KEY);
            } catch(e) {}
            location.reload(); 
        } 
    }
        ...
        targetItems.forEach(({ item, idx }) => {
            ...
            let optHtml = opts.map((o, i) => {
                if (String(o).includes('|')) {
                    return `<div class="review-opt-line"><b>選項 ${i+1}:</b> ${o.split('|').map((s, si)=>`(${si+1})${s}`).join(' ')}</div>`;
                }
                return `<div class="review-opt-line">${i+1}. ${o}</div>`;
            }).join('');
    ...
    function renderQuestion(index) {
            ...
            <div class="card question-card">
                ...
                <div class="question-body">
                    ...
                    <div class="answer-section" id="ans-section">
                        <h6 class="fw-bold mb-3">正確答案: <span class="text-blue">${Array.isArray(item.answer) ? item.answer.join(', ') : item.answer}</span></h6>
        ...
        if (opts.some(o => String(o).includes('|'))) {
            opts.forEach((opt, r) => {
                const correctSubIdx = parseAnswerToIndex(answers[r]);
    ...
    subjects = [
        {'dir': 'www/AI900', 'json': 'questions_AI900.json', 'title': 'AI-900', 'id': 'ai900', 'html': 'AI900.html'},
        {'dir': 'www/AZ900', 'json': 'questions_AZ900.json', 'title': 'AZ-900', 'id': 'az900', 'html': 'AZ900.html'},
        {'dir': 'www/Generative_AI', 'json': 'questions_Generative_AI_Foundations.json', 'title': 'Generative AI', 'id': 'genai', 'html': 'Generative_AI.html'},
        {'dir': 'www/ITS_AI', 'json': 'questions_ITS_AI.json', 'title': 'ITS AI', 'id': 'itsai', 'html': 'ITS_AI.html'},
        {'dir': 'www/ITS_Database', 'json': 'questions_ITS_Database.json', 'title': 'ITS Database', 'id': 'itsdb', 'html': 'ITS_Database.html'},
        {'dir': 'www/ITS_softdevelop', 'json': 'questions_ITS_csharp.json', 'title': 'ITS Software Development', 'id': 'itscs', 'html': 'ITS_softdevelop.html'},
        {'dir': 'www/ITS_Python', 'json': 'questions_ITS_python.json', 'title': 'ITS Python', 'id': 'itspy', 'html': 'ITS_Python.html'}
    ]
    ...
    for subj in subjects:
        try:
            ...
            target_html = os.path.join(subj['dir'], subj['html'])
            
            with open(target_html, 'wb') as f:
                # 1. 寫入頭部
                head = html_top.replace('REPLACE_TITLE', subj['title']).replace('REPLACE_TOTAL', str(total_count)).encode('utf-8')
                f.write(head)
                # 2. 寫入 JSON 數據
                f.write(json_cleaned)
                # 3. 寫入尾部
                foot = html_bottom.replace('REPLACE_TITLE', subj['title']).replace('REPLACE_SUBJECT_ID', subj['id']).encode('utf-8')
                f.write(foot)
            
            print(f"Practice Upgraded: {target_html} ({total_count} items)")
        except Exception as e:
            print(f"Failed {subj['dir']}: {e}")

if __name__ == "__main__":
    upgrade_practice_pages()
