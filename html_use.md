...
## 11. CSV 轉 JSON 表格工具 (`convert_csv_to_json.py`)
為了快速建立大型指令表，系統提供自動化轉換腳本，可將 CSV 資料轉換為具備 V3.5+ 標準樣式的 JSON 陣列。

### 11.1 CSV 格式要求
| 欄位 1 (指令) | 欄位 2 (語法) | 欄位 3 (說明) |
| :--- | :--- | :--- |
| **一、 DDL 分類** | (留空) | (留空) |
| CREATE | CREATE TABLE... | 建立新表... |

*   **自動識別分類**：若第二、三欄為空，系統會自動將該列轉為灰色背景的 `category-title`。
*   **字體處理**：第二欄（語法）會自動套用 `code-font` (Cascadia Code) 並改為深藍色。

### 11.2 執行與使用
1. 執行：`python convert_csv_to_json.py data.csv`
2. 產出：`data.json` (內容為純 HTML 字串陣列)。
3. 使用：將 `data.json` 內的內容直接貼入題庫 JSON 的相關欄位即可。

### 11.3 產出樣式規格
*   **類別**：`.q-table w-100 t-center` (預設全寬、置中)。
*   **比例**：`15% / 42.5% / 42.5%` (固定佈局，嚴格遵守比例)。
*   **邊框**：黑色實線 (`#000`)。
