...
def fix_questions_json_robust():
    path = 'www/ITS_Python/questions_ITS_python.json'
    if not os.path.exists(path):
        print(f"File not found: {path}")
        return

    # 先嘗試用 utf-8 讀取，如果失敗嘗試 cp950 (Windows 常見編碼)
    content = None
    for enc in ['utf-8', 'cp950', 'big5']:
        try:
            with open(path, 'r', encoding=enc) as f:
                content = json.load(f)
            print(f"Successfully loaded JSON with {enc}")
            break
        except Exception:
            continue
    
    if content is None:
        print("Failed to load JSON with available encodings.")
        return
...
● 因此，選項 2 和 選項 4 是正確的。"
    ...
    if found:
        ...
        print("Successfully updated ID 27 and saved as UTF-8.")
    else:
        print("ID 27 not found.")

if __name__ == "__main__":
    fix_questions_json_robust()
