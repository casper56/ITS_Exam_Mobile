...
for i, line in enumerate(lines):
    if 'setTimeout(drawLines, 150);' in line:
        start_line = i
    if 'function parseAnswerToIndex(val)' in line:
        end_line = i
...
'
    ...
    # 組合新內容
    final_lines = lines[:start_line + 1] + new_js + lines[end_line:]
    with open(file_path, 'w', encoding='utf-8') as f:
        f.writelines(final_lines)
    print("SUCCESS: Full rebuild of matching logic completed.")
else:
