...
def emergency_restore_v2():
    base_dir = 'www'
    
    # Define the COMPLETE HTML template here to avoid parsing issues
...
<div class="main-wrapper">
    ...
    <nav class="sidebar" id="sidebar">
        <div class="sidebar-header d-flex justify-content-between align-items-center">
            ...
            <small class="text-white-50" id="progress-stats">0/{total_count}</small>
        </div>
...
""";
    ...
    for subdir in os.listdir(base_dir):
        ...
        html_filename = f"{subdir}.html"
        html_path = os.path.join(subdir_path, html_filename)
        
        print(f"Rebuilding {html_path}...")
        ...
        final_html = html_template_start.replace('{display_title}', display_title) \
                                        .replace('{total_count}', str(len(data))) \
                                        .replace('{json_data}', json_str) \
                                        .replace('{storage_key}', f"{safe_name}_visited_v1") \
                                        .replace('{index_key}', f"{safe_name}_current_idx_v1")
                                        
        with open(html_path, 'w', encoding='utf-8') as f:
            f.write(final_html)
            
    print("All pages completely rebuilt.")

if __name__ == "__main__":
    emergency_restore_v2()
