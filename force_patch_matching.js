...
function patchFile(filePath) {
    ...
    if (sIdx !== -1 && eIdx !== -1) {
        let jsonStr = content.substring(sIdx + startMarker.length - 1, eIdx + 1);
        try {
            ...
            const newContent = content.substring(0, sIdx + startMarker.length - 1) + 
                               JSON.stringify(data, null, 4) + 
                               content.substring(eIdx + 1);
            fs.writeFileSync(filePath, newContent, 'utf-8');
            console.log('SUCCESS SYNC:', filePath);
        } catch(e) { console.log('ERROR:', filePath, e.message); }
        ...
        console.log('SUCCESS JSON:', filePath);
    }
}
