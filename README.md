...
## 🎨 如何自訂題目文字顏色
如果您需要強調題目中的特定文字，可以直接在 JSON 檔案的 `question` 欄位中使用 HTML 的 `<span>` 標籤與 `style` 屬性。

**範例：將「資料型態」四個字變成藍色**
```json
"question": "1. 【CH01-1】<span style='color: blue'>資料型態</span>的檢視：<br>..."
```
...
1.  **英文顏色名稱** (直觀簡單)：
    ...
    - `green` (綠)
    - `orange` (橘)
2.  **十六進位色碼 (Hex Codes)** (精準控制)：
    - `#0d6efd` (Bootstrap 預設藍)
    - `#330909` (您指定的深褐色)
    - `#FF5733` (鮮豔橘紅)
3.  **RGB 色碼**：
    - `rgb(51, 9, 9)` (等同於 #330909)
    - `rgb(255, 0, 0)` (純紅)

**修改範例 (使用 Hex 色碼)：**
```json
"question": "1. <span style='color: #330909'>這段文字會變成深褐色</span>..."
```

**⚠️ 注意事項：**
修改 JSON 後，請務必執行 `python final_clean_repair.py` 進行同步，變更才會生效。
