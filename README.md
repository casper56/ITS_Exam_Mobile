...
### 1. 極致平衡出題演算法
*   **自動分類**：腳本會根據題目特徵（如 `try`、`import`、`def` 等）自動將 149 題題庫劃分為以下 **7 大領域**：
    *   **D1_資料型別與運算子** (基礎語法)
    *   **D2_流程控制** (If/Loop)
    *   **D3_輸入與輸出** (Input/Print/File)
    *   **D4_程式碼文件與結構** (Function/Class/Comment)
    *   **D5_錯誤處理與測試** (Try-Except/Unittest)
    *   **D6_模組與工具** (Standard Libraries)
    *   **D7_進階題(APCS/演算法)** (Recursion/Stack/Queue)
